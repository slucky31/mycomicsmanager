...
public class BookTests
{
    ...
    [Fact]
    public void AddReadingDate_Should_ReturnReadingDateWithCorrectBookId()
    {
        // Arrange
        var book = Book.Create("Preacher", "Volume 1", "9781563892257");
    ...
    [Fact]
    public void AddReadingDate_Should_AllowNegativeRating()
    {
        // Arrange
        var book = Book.Create("Test Series", "Test Book", "9781234567890");

        // Act
        book.AddReadingDate(DateTime.UtcNow, -5);
    ...
    [Fact]
    public void AddReadingDate_Should_AllowVeryHighRating()
    {
        // Arrange
        var book = Book.Create("Test Series", "Test Book", "9781234567890");

        // Act
        book.AddReadingDate(DateTime.UtcNow, 1000);

        // Assert
        book.ReadingDates.Should().HaveCount(1);
        book.ReadingDates[0].Rating.Should().Be(1000);
    }

    [Fact]
    public void RemoveReadingDate_Should_MaintainOrderOfRemainingReadingDates()
    {
        // Arrange
        var book = Book.Create("Test Series", "Test Book", "9781234567890");
        ...
        book.AddReadingDate(date1, 3);
        var middleReading = book.AddReadingDate(date2, 4);
        book.AddReadingDate(date3, 5);

        // Act
        book.RemoveReadingDate(middleReading.Id);

        // Assert
        book.ReadingDates.Should().HaveCount(2);
        book.ReadingDates[0].Date.Should().Be(date1);
        book.ReadingDates[0].Rating.Should().Be(3);
        book.ReadingDates[1].Date.Should().Be(date3);
        book.ReadingDates[1].Rating.Should().Be(5);
    }
}