...
public class AddReadingDateCommandHandlerTests
{
    ...
    [Fact]
    public async Task Handle_Should_AddMultipleReadingDates_ToSameBook()
    {
        // Arrange
        var book = Book.Create("Serie", "Title", "978-3-16-148410-0");
        _bookRepositoryMock.GetByIdAsync(book.Id).Returns(book);

        var command1 = new AddReadingDateCommand(book.Id, 3);
        var command2 = new AddReadingDateCommand(book.Id, 5);

        // Act
        var result1 = await _handler.Handle(command1, default);
        var result2 = await _handler.Handle(command2, default);
    ...
    [Fact]
    public async Task Handle_Should_PreserveExistingReadingDates_WhenAddingNewReadingDate()
    {
        // Arrange
        var book = Book.Create("Serie", "Title", "978-3-16-148410-0");
        var existingReadingDate = book.AddReadingDate(DateTime.UtcNow.AddDays(-30), 4);
        _bookRepositoryMock.GetByIdAsync(book.Id).Returns(book);
        ...
        result.IsSuccess.Should().BeTrue();
        book.ReadingDates.Should().HaveCount(2);
        book.ReadingDates.Should().Contain(rd => rd.Id == existingReadingDate.Id && rd.Rating == 4);
        book.ReadingDates.Should().Contain(rd => rd.Rating == 5);
    }

    [Fact]
    public async Task Handle_Should_AllowZeroRating()
    {
        // Arrange
        var book = Book.Create("Serie", "Title", "978-3-16-148410-0");
        var command = new AddReadingDateCommand(book.Id, 0);
        _bookRepositoryMock.GetByIdAsync(command.BookId).Returns(book);
        ...
        result.IsSuccess.Should().BeTrue();
        book.ReadingDates.Should().HaveCount(1);
        book.ReadingDates[0].Rating.Should().Be(0);
    }

    [Fact]
    public async Task Handle_Should_AllowMaximumRating()
    {
        // Arrange
        var book = Book.Create("Serie", "Title", "978-3-16-148410-0");
        var command = new AddReadingDateCommand(book.Id, 5);
        ...
        result.IsSuccess.Should().BeTrue();
        book.ReadingDates.Should().HaveCount(1);
        book.ReadingDates[0].Rating.Should().Be(5);
    }
}