...
- PostgreSQL tournant en Docker : l'image officielle embarque déjà `psql`, `pg_dump` et `pg_restore`, pas besoin de les installer sur l'hôte — toutes les commandes ci-dessous passent par `docker exec`/`docker run`

```bash
docker ps   # repère le nom/l'ID du conteneur Postgres, ex: pg_main
docker exec pg_main psql -U postgres -c "SHOW server_version;"   # version du serveur self-hosted
```

⚠️ `pg_dump` doit être de version **≥** celle du serveur qu'il dump. Neon tourne en Postgres 17 — si `pg_main` est en 16 (ou toute version plus ancienne que Neon), son `pg_dump` refusera de dumper Neon (`error: aborting because of server version mismatch`). Dans ce cas, l'étape 2 utilise un conteneur jetable `postgres:17` au lieu de `pg_main` pour le dump ; ajuste le tag à la version exacte de Neon si besoin (`SELECT version();` sur Neon).
...
Si `pg_main` a la même version majeure que Neon (17), il peut dumper directement — pas besoin de récupérer le dump sur l'hôte, la restauration (étape 3) se fait sur `localhost` dans ce même conteneur :

```bash
docker exec pg_main pg_dump "postgresql://<user>:<pass>@<neon-host>/<db>?sslmode=require&channel_binding=require" \
...
**Si `pg_main` est plus ancien que Neon** (version mismatch), utilise un conteneur jetable `postgres:17` à la place, monté sur un dossier de l'hôte pour récupérer le fichier :

```bash
cd ~/postgres-config/dump
sudo docker run --rm -v "$(pwd)":/dump postgres:17 \
  pg_dump "postgresql://<user>:<pass>@<neon-host>/<db>?sslmode=require&channel_binding=require" \
  --format=custom --no-owner --no-privileges \
  --file=/dump/mycomicsmanager_prod.dump
```
...
Si le dump a été fait par `pg_main` (localhost) :

```bash
docker exec pg_main pg_restore --no-owner --no-privileges \
  --dbname="postgresql://mycomicsmanager:<mot-de-passe-fort>@localhost:5432/mycomicsmanager_prod" \
  /tmp/mycomicsmanager_prod.dump
```

Si le dump a été fait par le conteneur jetable `postgres:17`, réutilise-le pour le restore (un `pg_restore` plus récent que le serveur cible fonctionne sans problème — c'est l'inverse qui ne l'est pas) :

```bash
# si pg_main publie le port 5432 sur l'hôte (vérifier avec `docker port pg_main`)
sudo docker run --rm --network host -v "$(pwd)":/dump postgres:17 \
  pg_restore --no-owner --no-privileges \
  --dbname="postgresql://mycomicsmanager:<mot-de-passe-fort>@localhost:5432/mycomicsmanager_prod" \
  /dump/mycomicsmanager_prod.dump

# sinon, attache-le au réseau docker de pg_main (`docker network ls`)
sudo docker run --rm --network <nom-du-reseau> -v "$(pwd)":/dump postgres:17 \
  pg_restore --no-owner --no-privileges \
  --dbname="postgresql://mycomicsmanager:<mot-de-passe-fort>@pg_main:5432/mycomicsmanager_prod" \
  /dump/mycomicsmanager_prod.dump
```
