...
## Repository Snapshot
- MyComicsManager is a .NET solution built around a strict layered architecture: `Domain` (entities, value objects, errors) â†’ `Application` (CQRS handlers, orchestrators, abstractions) â†’ `Persistence` (EF Core/PostgreSQL infrastructure) â†’ `Web` (MudBlazor UI, endpoints, configuration).
...
- Uphold architecture testsâ€”avoid cross-layer references or shortcuts that would fail `tests/Architecture.Tests`.
...
Use these guardrails when generating code, docs, or tests so suggestions stay aligned with the projectâ€™s architecture and tooling.
