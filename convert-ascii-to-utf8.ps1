...
# Usage: .\convert-ascii-to-utf8.ps1
# Usage avec test: .\convert-ascii-to-utf8.ps1 -DryRun
...
Write-Host "  Conversion ASCII → UTF-8 sans BOM" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
function Convert-FileToUTF8NoBOM {
    ...
    try {
        # On veut convertir TOUT en UTF-8 sans BOM explicitement
        # Même les fichiers ASCII pour avoir un encodage cohérent
...
if ($DryRun) {
    ...
    Write-Host "Déjà UTF-8             : $($stats.Skipped)" -ForegroundColor Green
}
else {
    Write-Host "Fichiers convertis     : $($stats.Converted)" -ForegroundColor Green
    Write-Host "Déjà UTF-8             : $($stats.Skipped)" -ForegroundColor DarkGray
}
...
if ($DryRun) {
    ...
    Write-Host "  Exécutez sans -DryRun pour convertir tous les fichiers ASCII en UTF-8" -ForegroundColor Yellow
}
else {
    Write-Host "✓ Conversion terminée!" -ForegroundColor Green
    Write-Host "  Tous vos fichiers sont maintenant explicitement en UTF-8 sans BOM" -ForegroundColor Green
    ...
    if ($stats.Converted -gt 0) {
        ...
        Write-Host "     git commit -m `"Convert all ASCII files to explicit UTF-8 without BOM`"" -ForegroundColor DarkGray
    }
}
