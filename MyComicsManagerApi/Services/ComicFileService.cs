...
namespace MyComicsManagerApi.Services
{
    public class ComicFileService
    {
        ...
        public void SetAndExtractCoverImage(Comic comic)
        {
            ...
            try
            {
                string coverImagePath = ExtractImageFromCbz(comic, extractPath, 0);
        ...
        {
            // Normalizes the path.
            string extractPath = Path.GetFullPath(_applicationConfigurationService.GetPathIsbn());
        ...
        private string ExtractImageFromCbz(Comic comic, string extractPath, int imageIndex)
        {
            ...
            IOrderedEnumerable<ZipArchiveEntry> images = archive.Entries
                .Where(file =>
            ...
            string destinationPath = Path.GetFullPath(Path.Combine(extractPath,
            ...
            if (destinationPath.StartsWith(extractPath, StringComparison.Ordinal))
            {
                ...
                try
                {
                    using Image image = Image.Load(destinationPath);
                    Rectangle rectangle;
        ...
        public void ConvertComicFileToCbz(Comic comic)
        {
            ...
            string extension = GetArchiveType(comic);
            switch (extension)
            {
                case ".cbz":
                case ".zip":
            ...
            string archiveDirectoryPath = Path.Combine(tempDir, "archive");
            ...
            foreach (string file in extractedFiles)
            {
                ...
                string destFile = Path.GetFileNameWithoutExtension(file) + Path.GetExtension(file).ToLower();
                string destination = Path.Combine(archiveDirectoryPath, destFile);
            ...
            List<string> files = Directory.EnumerateFiles(archiveDirectoryPath, ".*", SearchOption.AllDirectories).ToList();
            ...
            if (comic.EbookPath != null)
            {
                ...
                List<string> filesToArchive = Directory.EnumerateFiles(archiveDirectoryPath, "*.*", SearchOption.AllDirectories)
                    .Where(file =>
                ...
                using ZipArchive archive = ZipFile.Open(comic.EbookPath, ZipArchiveMode.Create);
                foreach (string file in filesToArchive)
                {
                    ZipArchiveEntry entry = archive.CreateEntryFromFile(file, Path.GetFileName(file), CompressionLevel.Optimal);
        ...
        public void ConvertImagesToWebP(Comic comic)
        {
            ...
            List<string> filesToConvert = Directory.EnumerateFiles(tempDir, "*.*", SearchOption.AllDirectories)
                .Where(file =>
            ...
            foreach (string file in filesToConvert)
            {
                ...
                string webpConvertedFile = Path.ChangeExtension(file, ".webp");
            ...
            string destBackUp = Path.ChangeExtension(zipPath, ".old");
            if (File.Exists(zipPath))
            {
            ...
            List<string> filesToArchive = Directory.EnumerateFiles(tempDir, "*.*", SearchOption.AllDirectories)
                .Where(file =>
                    _extensionsFileArchive.Any(x => file.EndsWith(x, StringComparison.OrdinalIgnoreCase))).ToList();
            Log.Here().Information("Archivages des {NbFiles} fichiers", filesToArchive.Count);
        ...
        private void ExtractImagesFromCbz(Comic comic, string tempDir)
        {
            try
            {
                string comicEbookPath = GetComicFileAbsolutePath(comic);
        ...
        private void ExtractImagesFromPdf(string comicEbookPath, string tempDir)
        {
            ...
            foreach (UglyToad.PdfPig.Content.Page page in document.GetPages())
            {
                foreach (UglyToad.PdfPig.Content.IPdfImage image in page.GetImages())
                {
                    ...
                    {
                        ReadOnlySpan<byte> b = image.RawBytes;
                        string imageName = Path.Combine(tempDir, "P" + page.Number.ToString("D5") + ".jpg");
                        File.WriteAllBytes(imageName, b.ToArray());
                        Log.Here().Debug("Image with {Size} bytes on page {Page}. Location: {Image}", b.Length, page.Number, imageName);
                    }
                }
            }
        }
        ...
        public int GetNumberOfImagesInCbz(Comic comic)
        {
            string zipPath = GetComicEbookPath(comic, LibraryService.PathType.ABSOLUTE_PATH);
            using ZipArchive archive = ZipFile.OpenRead(zipPath);
            IEnumerable<ZipArchiveEntry> images = archive.Entries.Where(file =>
                _extensionsImageArchive.Any(x => file.FullName.EndsWith(x, StringComparison.OrdinalIgnoreCase)));
            return images.Count();
        }
        ...
        {
            string extractedText = await ReadInfoFromPage(comic, 0);
        ...
        private async Task<string> ReadInfoFromPage(Comic comic, int pageId)
        {
            ...
            string imagePath = ExtractImageFromCbz(comic, tempDir, pageId);
            Log.Here().Information("imagePath : {Path}", imagePath);

            string extractedText = await _computerVisionService.ReadTextFromLocalImage(imagePath);
        ...
        {
            string comicEbookPath = GetComicEbookPath(comic, LibraryService.PathType.ABSOLUTE_PATH);

            using FileStream zipToOpen = new FileStream(comicEbookPath, FileMode.Open);
            using ZipArchive archive = new ZipArchive(zipToOpen, ZipArchiveMode.Update);
            ZipArchiveEntry entry = archive.GetEntry("ComicInfo.xml");
            return entry != null;
        }
        ...
        public void DeleteFilesBeginningWithDots(Comic comic)
        {
            ...
            List<ZipArchiveEntry> entriesWithDots = archive.Entries.Where(file => file.Name[0] == '.').ToList();
            Log.Here().Information("Détection de {Count} fichier(s) commençant par un . à supprimer", entriesWithDots.Count);
            foreach (ZipArchiveEntry entry in entriesWithDots)
            {
                entry.Delete();
            }
        }
        ...
        public void AddComicInfoInComicFile(Comic comic)
        {
            ...
            XmlSerializer mySerializer = new XmlSerializer(typeof(ComicInfo));
        ...
        public Comic ExtractDataFromComicInfo(Comic comic)
        {
            ...
            ComicInfo comicInfo = (ComicInfo)serializer.Deserialize(reader);
        ...
        private Comic Move(Comic comic, string destination)
        {
            ...
            try
            {
                string absolutePath = GetComicFileAbsolutePath(comic);
                ...
                while (File.Exists(destination))
                {
                    ...
                    string fileName = Path.GetFileNameWithoutExtension(destination) + "-Duplicate" + Path.GetExtension(destination);
        ...
        public Comic MoveToLib(Comic comic, string relPathinLib)
        {
            ...
            string ebookPath = relPathinLib + comic.EbookName;
        ...
        public Comic MoveToError(Comic comic)
        {
            ...
            string destination = _applicationConfigurationService.GetPathImportErrors();
        ...
        {
            // Déplacement du fichier vers le répertoire d'erreurs d'import
            string destination = _applicationConfigurationService.GetPathFileImport();
            destination += comic.EbookName;
